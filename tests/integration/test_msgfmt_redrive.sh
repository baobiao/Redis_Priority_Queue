...
for e in $ENGINES; do
  ...
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}G" G 1 Priority 5 Payload PAY-G >/dev/null
  mG=$(printf '%020d:%s' 1 G)
  fcall "$e" msgfmt_dequeue 3 "$q" "$pfx" "$dl" 1000 30000 0 1 >/dev/null   # RA 0<1 -> leased, RA=1, RDT=1000
  fcall "$e" msgfmt_nack 1 "${pfx}G" 1 >/dev/null                          # RA=1, DirtyBit=0
  fcall "$e" msgfmt_dequeue 3 "$q" "$pfx" "$dl" 2000 30000 0 1 >/dev/null   # RA=1>=1 -> dead-lettered
  expect "G is in the DLQ before redrive" "5" "$(engine_cli "$e" ZSCORE "$dl" "$mG")"

  out=$(fcall "$e" msgfmt_redrive 3 "$dl" "$q" "${pfx}G" "$mG")
  ...
  out=$(fcall "$e" msgfmt_dequeue 3 "$q" "$pfx" "$dl" 5000 30000 0 1)
  ...
  fcall "$e" msgfmt_enqueue 2 "$q2" "${pfx2}H" H 1 Priority 5 Payload PAY-H >/dev/null
  mH=$(printf '%020d:%s' 1 H)
  engine_cli "$e" ZADD "$dl2" 5 "$mH" >/dev/null            # H present in BOTH source and DLQ
  out=$(fcall "$e" msgfmt_redrive 3 "$dl2" "$q2" "${pfx2}H" "$mH" 2>&1 || true)
