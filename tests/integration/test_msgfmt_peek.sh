...
for e in $ENGINES; do
  ...
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}A" A 1 Priority 10 Payload PAY-A >/dev/null
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}B" B 2 Priority 5  Payload PAY-B >/dev/null
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}C" C 3 Priority 10 Payload PAY-C >/dev/null
  ...
  deq=$(fcall "$e" msgfmt_dequeue 2 "$q" "$pfx" 1000 30000)
  ...
  out=$(fcall_ro "$e" msgfmt_peek 2 "$q" "$pfx" 2000 30000 3)
  ...
  out=$(fcall_ro "$e" msgfmt_peek 2 "$q" "$pfx" 2000 30000 100)
  ...
  out=$(fcall_ro "$e" msgfmt_peek 2 "$eq" "$epfx" 1000 30000)
  expect "empty queue single peek -> null" "" "$out"
  out=$(fcall_ro "$e" msgfmt_peek 2 "$eq" "$epfx" 1000 30000 5)
  ...
  fcall "$e" msgfmt_enqueue 2 "$lq" "${lpfx}Z" Z 1 Priority 5 Payload PAY-Z >/dev/null
  fcall "$e" msgfmt_dequeue 2 "$lq" "$lpfx" 1000 30000 >/dev/null   # lease Z (unexpired)
  out=$(fcall_ro "$e" msgfmt_peek 2 "$lq" "$lpfx" 1001 30000)
  ...
  fcall "$e" msgfmt_enqueue 2 "$dq" "${dpfx}D" D 1 Priority 5 Payload PAY-D >/dev/null
  mD=$(printf '%020d:%s' 1 D)
  engine_cli "$e" DEL "${dpfx}D" >/dev/null                        # Hash gone -> dangling member
  out=$(fcall_ro "$e" msgfmt_peek 2 "$dq" "$dpfx" 1000 30000 5)
  ...
  fcall "$e" msgfmt_enqueue 2 "$dl" "${dlpfx}W" W 1 Priority 9 Payload PAY-W >/dev/null
  out=$(fcall_ro "$e" msgfmt_peek 2 "$dl" "$dlpfx" 1000 30000)
