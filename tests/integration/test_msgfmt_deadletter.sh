...
for e in $ENGINES; do
  ...
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}A" A 1 Priority 5 Payload PAY-A >/dev/null
  mA=$(printf '%020d:%s' 1 A)
  # Two deliver-then-nack cycles bring ReadAttempts to 2 (= cap).
  fcall "$e" msgfmt_dequeue 3 "$q" "$pfx" "$dl" 1000 30000 0 2 >/dev/null
  fcall "$e" msgfmt_nack 1 "${pfx}A" 1 >/dev/null
  fcall "$e" msgfmt_dequeue 3 "$q" "$pfx" "$dl" 2000 30000 0 2 >/dev/null
  fcall "$e" msgfmt_nack 1 "${pfx}A" 2 >/dev/null
  ...
  out=$(fcall "$e" msgfmt_dequeue 3 "$q" "$pfx" "$dl" 3000 30000 0 2)
  ...
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}B" B 1 Priority 5 Payload PAY-B >/dev/null
  out=$(fcall "$e" msgfmt_dequeue 3 "$q" "$pfx" "$dl" 1000 30000 0 2)
  ...
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}C" C 1 Priority 5 Payload PAY-C >/dev/null
  mC=$(printf '%020d:%s' 1 C)
  fcall "$e" msgfmt_dequeue 3 "$q" "$pfx" "$dl" 1000 30000 0 2 >/dev/null   # RA=1, in-flight
  fcall "$e" msgfmt_nack 1 "${pfx}C" 1 >/dev/null                          # RA=1, DirtyBit=0
  fcall "$e" msgfmt_dequeue 3 "$q" "$pfx" "$dl" 2000 30000 0 2 >/dev/null   # RA=2, in-flight, RDT=2000
  ...
  out=$(fcall "$e" msgfmt_dequeue 3 "$q" "$pfx" "$dl" 2001 30000 0 2)
  ...
  out=$(fcall "$e" msgfmt_dequeue 3 "$q" "$pfx" "$dl" 40000 30000 0 2)
  ...
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}E" E 1 Priority 5 Payload PAY-E >/dev/null
  mE=$(printf '%020d:%s' 1 E)
  fcall "$e" msgfmt_dequeue 2 "$q" "$pfx" 1000 30000 >/dev/null
  fcall "$e" msgfmt_nack 1 "${pfx}E" 1 >/dev/null
  fcall "$e" msgfmt_dequeue 2 "$q" "$pfx" 2000 30000 >/dev/null
  fcall "$e" msgfmt_nack 1 "${pfx}E" 2 >/dev/null
  # ReadAttempts=2, but the 2-key call has no cap -> must lease, never dead-letter.
  out=$(fcall "$e" msgfmt_dequeue 2 "$q" "$pfx" 3000 30000)
  ...
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}F" F 1 Priority 7 Payload PAY-F >/dev/null
  mF=$(printf '%020d:%s' 1 F)
  engine_cli "$e" ZADD "$dl" 7 "$mF" >/dev/null            # simulate prior DLQ presence
  fcall "$e" msgfmt_dequeue 3 "$q" "$pfx" "$dl" 1000 30000 0 2 >/dev/null
  fcall "$e" msgfmt_nack 1 "${pfx}F" 1 >/dev/null
  fcall "$e" msgfmt_dequeue 3 "$q" "$pfx" "$dl" 2000 30000 0 2 >/dev/null
  fcall "$e" msgfmt_nack 1 "${pfx}F" 2 >/dev/null
  fcall "$e" msgfmt_dequeue 3 "$q" "$pfx" "$dl" 3000 30000 0 2 >/dev/null   # dead-letter F (already in DLQ)
