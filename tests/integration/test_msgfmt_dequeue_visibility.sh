...
for e in $ENGINES; do
  ...
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}v" v 1 Priority 5 Payload pv >/dev/null
  ...
  out=$(fcall "$e" msgfmt_ack 2 "$q" "${pfx}v" "$member" 1 2>&1 || true)
  expect_contains "stale ack rejected -> EFENCED" "EFENCED" "$out"
  out=$(fcall "$e" msgfmt_nack 1 "${pfx}v" 1 2>&1 || true)
  ...
  out=$(fcall "$e" msgfmt_ack 2 "$q" "${pfx}v" "$member" 2)
