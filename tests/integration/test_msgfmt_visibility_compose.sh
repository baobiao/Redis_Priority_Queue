...
for e in $ENGINES; do
  ...
  fcall "$e" msgfmt_create 1 "q:{cm}:m:1" Priority 5 Payload hi VisibleAt 90000 >/dev/null
  out=$(fcall_ro "$e" msgfmt_read 1 "q:{cm}:m:1")
  ...
  out=$(fcall_ro "$e" msgfmt_read 1 "${pfx}old")
  ...
  out=$(fcall "$e" msgfmt_dequeue 2 "$q" "$pfx" 1 30000)
  ...
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}F" F 1 Priority 5 Payload PAY-F VisibleAt 8000 >/dev/null
  out=$(fcall_ro "$e" msgfmt_peek 2 "$q" "$pfx" 7999 30000)
  expect "single peek skips not-yet-visible -> null" "" "$out"
  out=$(fcall_ro "$e" msgfmt_peek 2 "$q" "$pfx" 7999 30000 10)
  ...
  out=$(fcall_ro "$e" msgfmt_peek 2 "$q" "$pfx" 8000 30000)
  ...
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}M" M 1 Priority 5 Payload PAY-M >/dev/null
  mM=$(printf '%020d:%s' 1 M)
  fcall "$e" msgfmt_dequeue 3 "$q" "$pfx" "$dl" 1000 30000 0 1 >/dev/null   # RA 0<1 -> leased, RA=1
  fcall "$e" msgfmt_nack 1 "${pfx}M" 1 5000 >/dev/null                     # RA=1(=cap), hidden until 5000
  out=$(fcall "$e" msgfmt_dequeue 3 "$q" "$pfx" "$dl" 4999 30000 0 1)
  ...
  fcall "$e" msgfmt_dequeue 3 "$q" "$pfx" "$dl" 5000 30000 0 1 >/dev/null   # now visible + over cap -> DLQ
  ...
  out=$(fcall "$e" msgfmt_redrive 3 "$dl" "$q" "${pfx}M" "$mM")
