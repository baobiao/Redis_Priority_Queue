...
# msgfmt_reap permanently removes (member + Hash) entries older than the retention
# window and keeps within-window ones; it is bounded by `limit` (truncated flag);
# it cleans dangling members; and msgfmt_redrive clears DeadLetteredAt.
...
for e in $ENGINES; do
  ...
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}7" 7 7 Priority 5 Payload job >/dev/null
  m7=$(printf '%020d:%s' 7 7)
  fcall "$e" msgfmt_dequeue 3 "$q" "$pfx" "$dl" 1000 30000 0 1 >/dev/null   # RA 0<1 -> leased, RA=1
  fcall "$e" msgfmt_nack 1 "${pfx}7" 1 >/dev/null
  fcall "$e" msgfmt_dequeue 3 "$q" "$pfx" "$dl" 100000 30000 0 1 >/dev/null # RA=1>=1 -> dead-lettered at now=100000
  ...
  out=$(fcall "$e" msgfmt_reap 2 "$dl" "$pfx" 120000 30000 100)
  ...
  out=$(fcall "$e" msgfmt_reap 2 "$dl" "$pfx" 140000 30000 100)
  ...
  for i in 1 2 3; do
    fcall "$e" msgfmt_create 1 "${pfx2}$i" Priority 5 Payload "p$i" DeadLetteredAt 100000 >/dev/null
    engine_cli "$e" ZADD "$dl2" 5 "$(printf '%020d:%s' "$i" "$i")" >/dev/null
  done
  out=$(fcall "$e" msgfmt_reap 2 "$dl2" "$pfx2" 200000 1000 2)
  ...
  fcall "$e" msgfmt_reap 2 "$dl2" "$pfx2" 200000 1000 2 >/dev/null
  ...
  out=$(fcall "$e" msgfmt_reap 2 "$dl3" "$pfx3" 200000 1000 100)
  ...
  fcall "$e" msgfmt_create 1 "${pfx4}5" Priority 5 Payload z DeadLetteredAt 100000 >/dev/null
  m5=$(printf '%020d:%s' 5 5)
  engine_cli "$e" ZADD "$dl4" 5 "$m5" >/dev/null
  fcall "$e" msgfmt_redrive 3 "$dl4" "$q4" "${pfx4}5" "$m5" >/dev/null
