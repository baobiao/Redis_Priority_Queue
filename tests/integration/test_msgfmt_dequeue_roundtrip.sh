...
for e in $ENGINES; do
  ...
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}a" a 1 Priority 10 Payload task-a >/dev/null
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}b" b 2 Priority 5  Payload task-b >/dev/null
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}c" c 3 Priority 10 Payload task-c >/dev/null

  # --- Priority then FIFO order ---
  d1=$(fcall "$e" msgfmt_dequeue 2 "$q" "$pfx" 1000 30000)
  expect_contains "1st acquire = highest priority (b, pri5)" "task-b" "$d1"
  d2=$(fcall "$e" msgfmt_dequeue 2 "$q" "$pfx" 1000 30000)
  expect_contains "2nd acquire = a (pri10, seq1 before c)" "task-a" "$d2"
  d3=$(fcall "$e" msgfmt_dequeue 2 "$q" "$pfx" 1000 30000)
  expect_contains "3rd acquire = c (pri10, seq3)" "task-c" "$d3"

  # --- All in-flight -> null (distinct from an empty payload) ---
  d4=$(fcall "$e" msgfmt_dequeue 2 "$q" "$pfx" 1000 30000)
  ...
  ackout=$(fcall "$e" msgfmt_ack 2 "$q" "${pfx}b" "$(printf '%020d:%s' 2 b)" 1)
  expect "ack b returns OK" "OK" "$ackout"
  expect "b hash deleted" "NOTFOUND" "$(fcall_ro "$e" msgfmt_read 1 "${pfx}b")"
  expect "queue cardinality drops by 1" "$((before - 1))" "$(engine_cli "$e" ZCARD "$q")"
  expect "re-ack b is idempotent NOOP" "NOOP" "$(fcall "$e" msgfmt_ack 2 "$q" "${pfx}b" "$(printf '%020d:%s' 2 b)" 1)"

  # --- nack releases a; DirtyBit back to 0, ReadDateTime/ReadAttempts retained ---
  nackout=$(fcall "$e" msgfmt_nack 1 "${pfx}a" 1)
  ...
  d5=$(fcall "$e" msgfmt_dequeue 2 "$q" "$pfx" 2000 30000)
