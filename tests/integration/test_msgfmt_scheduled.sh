...
for e in $ENGINES; do
  ...
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}A" A 1 Priority 5 Payload PAY-A VisibleAt 5000 >/dev/null
  ...
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}H" H 1 Priority 5  Payload PAY-H VisibleAt 5000 >/dev/null  # higher prio, hidden
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}L" L 2 Priority 10 Payload PAY-L VisibleAt 0    >/dev/null  # lower prio, visible
  out=$(fcall "$e" msgfmt_dequeue 2 "$q" "$pfx" 1000 30000)
  ...
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}Z" Z 1 Priority 5 Payload PAY-Z VisibleAt 0 >/dev/null
  out=$(fcall "$e" msgfmt_dequeue 2 "$q" "$pfx" 1 30000)
  expect_contains "explicit VisibleAt=0 is immediately visible" "PAY-Z" "$out"
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}Y" Y 2 Priority 5 Payload PAY-Y >/dev/null   # no VisibleAt field
  out=$(fcall "$e" msgfmt_dequeue 2 "$q" "$pfx" 1 30000)
