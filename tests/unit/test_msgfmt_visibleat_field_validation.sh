...
# MSGFMT EINVAL: VisibleAt, writing nothing. Spec: FR-001/007.
set -euo pipefail
...
for e in $ENGINES; do
  ...
  for v in 0 1 1000 "$MAX"; do
    out=$(fcall_ro "$e" msgfmt_validate 0 VisibleAt "$v")
  ...
  for bad in -1 1.5 abc 9007199254740994; do
    out=$(fcall_ro "$e" msgfmt_validate 0 VisibleAt "$bad" 2>&1 || true)
  ...
  out=$(fcall "$e" msgfmt_create 1 "q:{vf}:m:1" Priority 5 VisibleAt -5 2>&1 || true)
  ...
  out=$(fcall "$e" msgfmt_enqueue 2 "pq:{vf}" "pq:{vf}:m:1" 1 1 Priority 5 VisibleAt notanum 2>&1 || true)
  ...
  fcall "$e" msgfmt_create 1 "q:{vf}:m:2" Priority 5 VisibleAt 12345 >/dev/null
