#!/usr/bin/env bash
# [US3] Unit: msgfmt_redrive input validation + fail-before-write. Bad key count /
# empty member / tag mismatch, and a dangling or malformed message Hash -> the
# correct MSGFMT E... results; nothing is written on any failure. Spec: FR-013..017.
set -euo pipefail
...
for e in $ENGINES; do
  ...
  # Bad key count.
  out=$(fcall "$e" msgfmt_redrive 2 "$dl" "$q" "m" 2>&1 || true)
  expect_contains "2 keys -> EKEYS" "EKEYS" "$out"

  # Empty member.
  out=$(fcall "$e" msgfmt_redrive 3 "$dl" "$q" "${pfx}X" "" 2>&1 || true)
  expect_contains "empty member -> EARGS" "EARGS" "$out"

  # Tag mismatch.
  out=$(fcall "$e" msgfmt_redrive 3 "dlq:{uv}" "pq:{nope}" "pq:{uv}:m:X" "anything" 2>&1 || true)
  ...
  out=$(fcall "$e" msgfmt_redrive 3 "$dl" "$q" "${pfx}X" "$mX" 2>&1 || true)
  ...
  out=$(fcall "$e" msgfmt_redrive 3 "$dl" "$q" "${pfx}Y" "$mY" 2>&1 || true)
