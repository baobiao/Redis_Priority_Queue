#!/usr/bin/env bash
# [US2] Unit: msgfmt_stats validation, read-only. Invalid now/timeout/max_scan, tag
# mismatch, and a non-zset queue return the correct MSGFMT E... errors; stats never
...
for e in $ENGINES; do
  ...
  for bad in -1 1.5 abc; do
    out=$(fcall_ro "$e" msgfmt_stats 2 "$q" "$pfx" "$bad" 30000 2>&1 || true)
    expect_contains "now=$bad -> ENOW" "ENOW" "$out"
  done
  for bad in 0 -5 abc; do
    out=$(fcall_ro "$e" msgfmt_stats 2 "$q" "$pfx" 1000 "$bad" 2>&1 || true)
    expect_contains "timeout=$bad -> ETMO" "ETMO" "$out"
  done
  for bad in -1 1.5 abc; do
    out=$(fcall_ro "$e" msgfmt_stats 2 "$q" "$pfx" 1000 30000 "$bad" 2>&1 || true)
    expect_contains "max_scan=$bad -> ESCAN" "ESCAN" "$out"
  done

  out=$(fcall_ro "$e" msgfmt_stats 2 "pq:{sv}" "pq:{nope}:m:" 1000 30000 2>&1 || true)
  expect_contains "tag mismatch -> ETAG" "ETAG" "$out"

  engine_cli "$e" DEL "str:{sv}" >/dev/null
  engine_cli "$e" SET "str:{sv}" notazset >/dev/null
  out=$(fcall_ro "$e" msgfmt_stats 2 "str:{sv}" "str:{sv}:m:" 1000 30000 2>&1 || true)
