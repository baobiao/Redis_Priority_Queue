#!/usr/bin/env bash
# [US1] Unit: msgfmt_reap validation + fail-before-write. Invalid now/retention/
# limit, tag mismatch, and a non-zset DLQ return the correct MSGFMT E... errors and
...
for e in $ENGINES; do
  ...
  fcall "$e" msgfmt_create 1 "${pfx}1" Priority 5 Payload x DeadLetteredAt 1 >/dev/null
  m1=$(printf '%020d:%s' 1 1)
  engine_cli "$e" ZADD "$dl" 5 "$m1" >/dev/null

  for bad in -1 1.5 abc; do
    out=$(fcall "$e" msgfmt_reap 2 "$dl" "$pfx" "$bad" 1000 10 2>&1 || true)
    expect_contains "now=$bad -> ENOW" "ENOW" "$out"
  done
  for bad in -1 1.5 abc; do
    out=$(fcall "$e" msgfmt_reap 2 "$dl" "$pfx" 100000 "$bad" 10 2>&1 || true)
    expect_contains "retention=$bad -> ERET" "ERET" "$out"
  done
  for bad in 0 -5 1.5 abc; do
    out=$(fcall "$e" msgfmt_reap 2 "$dl" "$pfx" 100000 1000 "$bad" 2>&1 || true)
    expect_contains "limit=$bad -> ELIMIT" "ELIMIT" "$out"
  done

  out=$(fcall "$e" msgfmt_reap 2 "dlq:{rv}" "pq:{nope}:m:" 100000 1000 10 2>&1 || true)
