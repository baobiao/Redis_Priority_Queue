...
for e in $ENGINES; do
  ...
  out=$(fcall "$e" msgfmt_dequeue 3 "pq:{c1}" "pq:{c1}:m:" "dlq:{c1}" 1000 30000 0 5)
  expect_contains "3-key dead-letter call still returns a handle" "hello" "$out"

  # Too many keys -> EKEYS.
  out=$(fcall "$e" msgfmt_dequeue 4 "pq:{c1}" "pq:{c1}:m:" "dlq:{c1}" "x:{c1}" 1000 30000 2>&1 || true)
  expect_contains "four keys -> EKEYS" "EKEYS" "$out"

  # Dead-letter mode with a missing/invalid cap -> ECAP.
  out=$(fcall "$e" msgfmt_dequeue 3 "pq:{c1}" "pq:{c1}:m:" "dlq:{c1}" 1000 30000 2>&1 || true)
  expect_contains "missing cap -> ECAP" "ECAP" "$out"
  out=$(fcall "$e" msgfmt_dequeue 3 "pq:{c1}" "pq:{c1}:m:" "dlq:{c1}" 1000 30000 0 0 2>&1 || true)
  expect_contains "cap<1 -> ECAP" "ECAP" "$out"

  # DLQ key not sharing the source hash tag -> ETAG.
  out=$(fcall "$e" msgfmt_dequeue 3 "pq:{c1}" "pq:{c1}:m:" "dlq:{other}" 1000 30000 0 5 2>&1 || true)
  ...
  out=$(fcall "$e" msgfmt_dequeue 3 "pq:{c1}" "pq:{c1}:m:" "dlq:{c1}" 1000 30000 0 5 2>&1 || true)
