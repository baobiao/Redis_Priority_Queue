#!/usr/bin/env bash
# [US3] Contract test for msgfmt_redrive: KEYS (DLQ, source, message Hash) + ARGV
...
for e in $ENGINES; do
  ...
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}k" k 1 Priority 5 Payload hello >/dev/null
  ...
  out=$(fcall "$e" msgfmt_redrive 3 "$dl" "$q" "${pfx}k" "$mk")
