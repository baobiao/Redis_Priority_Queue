#!/usr/bin/env bash
# [US2] Contract test for msgfmt_stats: KEYS=queue,prefix,[DLQ]; ARGV=now,timeout,
...
for e in $ENGINES; do
  ...
  fcall "$e" msgfmt_enqueue 2 "$q" "${pfx}1" 1 1 Priority 5 Payload x >/dev/null

  # Cheap tier via FCALL_RO (no-writes): returns depth/dlq_depth/front_priority.
  out=$(fcall_ro "$e" msgfmt_stats 3 "$q" "$pfx" "$dl" 1000 30000)
  ...
  out=$(fcall_ro "$e" msgfmt_stats 2 "$q" "$pfx" 1000 30000)
  expect_contains "2-key form valid" "depth" "$out"

  # Wrong key count -> EKEYS.
  out=$(fcall_ro "$e" msgfmt_stats 1 "$q" 1000 30000 2>&1 || true)
  expect_contains "one key -> EKEYS" "EKEYS" "$out"
  out=$(fcall_ro "$e" msgfmt_stats 4 "$q" "$pfx" "$dl" "x:{sc}" 1000 30000 2>&1 || true)
  expect_contains "four keys -> EKEYS" "EKEYS" "$out"

  # Bad args.
  out=$(fcall_ro "$e" msgfmt_stats 2 "$q" "$pfx" -1 30000 2>&1 || true)
  expect_contains "bad now -> ENOW" "ENOW" "$out"
  out=$(fcall_ro "$e" msgfmt_stats 2 "$q" "$pfx" 1000 0 2>&1 || true)
  expect_contains "bad timeout -> ETMO" "ETMO" "$out"
  out=$(fcall_ro "$e" msgfmt_stats 2 "$q" "$pfx" 1000 30000 -1 2>&1 || true)
  expect_contains "bad max_scan -> ESCAN" "ESCAN" "$out"

  # Tag mismatch -> ETAG.
  out=$(fcall_ro "$e" msgfmt_stats 2 "pq:{sc}" "pq:{other}:m:" 1000 30000 2>&1 || true)
  ...
  out=$(fcall_ro "$e" msgfmt_stats 2 "str:{sc}" "str:{sc}:m:" 1000 30000 2>&1 || true)
  ...
  out=$(fcall "$e" msgfmt_stats 2 "$q" "$pfx" 1000 30000)
