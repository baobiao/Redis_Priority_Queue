...
Audience: Redis/Valkey developers integrating with the `priority_queue` library.

The library uses exactly **two native data types**:

- a **Hash** per message (`pq_create`, `pq_read`, `pq_enqueue`, `pq_dequeue`,
  `pq_ack`, `pq_nack`; `pq_validate` checks field values only and touches no key);
- a **Sorted Set (ZSET)** per priority queue (`pq_enqueue`, `pq_dequeue`, `pq_ack`).

Every key is caller-supplied via `KEYS[]`, with one sanctioned exception: `pq_dequeue`
...
Source of truth: `src/functions/priority_queue.lua`. All values below are what the
...
(`KEYS[1]` for `pq_create`/`pq_read`; `KEYS[2]` for `pq_enqueue`). One Hash =
...
- **`DeadLetteredAt` `0` = not dead-lettered.** Set to the caller's `now` when `pq_dequeue` moves
  a message to the dead-letter queue; the retention reaper (§4a) ages entries out by it; `pq_redrive`
  resets it to `0`.
...
`pq_read` returns a flat `name value name value ...` array with each field decoded back to
its logical type:
...
the five fields returns a structured `PQ EMALFORMED: ...` error (never a partial message).
...
(`KEYS[1]` for `pq_enqueue`), holding one member per currently-enqueued message.
...
`pq_dequeue` does not remove a message from the queue; it **leases** it. The lease is
expressed entirely in three existing Hash fields — no new keys or fields:

| Field | Role while leased |
|-------|-------------------|
| `DirtyBit` | `1` = in-flight (leased); `0` = available. Acquire sets `1`; `pq_nack` resets to `0`; `pq_ack` deletes the message. |
...
**Availability**: `pq_dequeue` returns the front message that is either `DirtyBit=0`, or
`DirtyBit=1` with an **expired** lease (`now − ReadDateTime ≥ timeout`, the caller-supplied
visibility timeout). A message stays in the Sorted Set from enqueue until `pq_ack`; while
...
a lease generation. `pq_ack` / `pq_nack` proceed only when the message is in-flight
...
**Deliverability**: `pq_dequeue` (and `pq_peek` single mode) select a message only when it is
...
- **Retry backoff**: `pq_nack <token> <VisibleAt>` releases the lease **and** hides the message
  until `now ≥ VisibleAt` (retaining `ReadDateTime`/`ReadAttempts`).

`pq_redrive` resets `VisibleAt` to `0` (immediately visible). A not-yet-visible over-cap message
...
- **Dead-lettering** happens inside `pq_dequeue` when called with an optional DLQ key
  ...
  **member string preserved verbatim**. The message **Hash is never modified or moved**. The scan
  ...
  key/cap makes `pq_dequeue` behave exactly as Feature 003.
...
- **Redrive** (`pq_redrive`) is the reverse move for one message: `ZREM` from the DLQ, `ZADD`
...
- **Peek** (`pq_peek`) is a read-only inspection of any such queue and never mutates or removes
...
`DeadLetteredAt` field (the caller's `now`, stamped by `pq_dequeue`'s dead-letter move — the only
change to that otherwise index-only move). A message dead-lettered longer ago than a caller-supplied
**retention window** is permanently removed by **`pq_reap`**:
...
- **Priority-order caveat**: the DLQ stays Priority-scored (Feature 004 unchanged), so reap walks the
  ...
  unexpired high-priority ones; to fully drain, size `limit` to the DLQ depth (from `pq_stats`) or
...
### 4b. Observability — `pq_stats` (Feature 006)

`pq_stats` is a read-only (`no-writes`) snapshot of a queue, beyond `pq_peek`'s head view:
...
`pq_stats` never writes and never removes dangling members (that is reap's job).
...
done by `pq_dequeue` and `pq_peek` for a message discovered by scanning (whose id is not
knowable in advance). `pq_redrive` addresses a message whose id the caller knows, so it takes
the message Hash key **literally**. `pq_enqueue` takes two keys:

- `KEYS[1]` = the priority-queue **Sorted Set**;
- `KEYS[2]` = the message **Hash**.

`pq_dequeue` in dead-letter mode adds `KEYS[3]` = the **dead-letter Sorted Set**, and
`pq_redrive` takes the DLQ, the source queue, and the message Hash — in every case **all keys
...
`pq_dequeue` takes `KEYS[1]` = the queue Sorted Set and `KEYS[2]` = the message-key
...
tag (else `PQ ETAG`), so every constructed key — e.g. `pq:{q1}:m:42` — lands in the queue's
slot. `pq_peek` takes the same queue + prefix and constructs keys read-only. `pq_ack` and
`pq_redrive` receive the specific message Hash directly in `KEYS[]`; `pq_nack` receives
...
A **single, identical Lua source** (`src/functions/priority_queue.lua`) is designed to run
unmodified on:
