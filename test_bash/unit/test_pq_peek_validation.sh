#!/usr/bin/env bash
# [US2] Unit: pq_peek input validation and read-only malformed handling.
...
suite() {
  ...
  for bad in "-1" "1.5" "abc"; do
    out=$(fcall_ro "$e" pq_peek 2 "$q" "$pfx" "$bad" 30000 2>&1 || true)
    expect_contains "now=$bad -> ENOW" "ENOW" "$out"
  done
  for bad in "0" "-5" "x"; do
    out=$(fcall_ro "$e" pq_peek 2 "$q" "$pfx" 1000 "$bad" 2>&1 || true)
    expect_contains "timeout=$bad -> ETMO" "ETMO" "$out"
  done
  for bad in "0" "-2" "1.5" "nope"; do
    out=$(fcall_ro "$e" pq_peek 2 "$q" "$pfx" 1000 30000 "$bad" 2>&1 || true)
    expect_contains "count=$bad -> ECOUNT" "ECOUNT" "$out"
  done

  # Hash-tag mismatch.
  out=$(fcall_ro "$e" pq_peek 2 "pq:{pv}" "pq:{nope}:m:" 1000 30000 2>&1 || true)
  expect_contains "tag mismatch -> ETAG" "ETAG" "$out"

  # Non-zset queue key.
  engine_cli "$e" DEL "str:{pv}" >/dev/null
  engine_cli "$e" SET "str:{pv}" "not-a-zset" >/dev/null
  out=$(fcall_ro "$e" pq_peek 2 "str:{pv}" "str:{pv}:m:" 1000 30000 2>&1 || true)
