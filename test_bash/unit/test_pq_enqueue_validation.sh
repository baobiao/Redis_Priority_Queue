...
suite() {
  ...
  out=$(fcall "$e" pq_enqueue 2 "pq:{u1}" "pq:{u1}:m:x" x -1 2>&1 || true)
  expect_contains "negative sequence -> ESEQ" "ESEQ" "$out"

  out=$(fcall "$e" pq_enqueue 2 "pq:{u1}" "pq:{u1}:m:x" x 1.5 2>&1 || true)
  expect_contains "non-integer sequence -> ESEQ" "ESEQ" "$out"

  out=$(fcall "$e" pq_enqueue 2 "pq:{u1}" "pq:{u1}:m:x" x abc 2>&1 || true)
  expect_contains "non-numeric sequence -> ESEQ" "ESEQ" "$out"

  out=$(fcall "$e" pq_enqueue 2 "pq:{u1}" "pq:{u1}:m:x" x 1 Priority foo 2>&1 || true)
  expect_contains "bad Priority -> EINVAL Priority" "EINVAL: Priority" "$out"

  out=$(fcall "$e" pq_enqueue 2 "pq:{u1}" "pq:{u1}:m:x" x 1 Color red 2>&1 || true)
  expect_contains "unknown field -> EFIELD Color" "EFIELD: Color" "$out"

  out=$(fcall "$e" pq_enqueue 2 "pq:{u1}" "pq:{u1}:m:x" x 1 Priority 1 Priority 2 2>&1 || true)
  expect_contains "duplicate field -> EDUP Priority" "EDUP: Priority" "$out"

  out=$(fcall "$e" pq_enqueue 2 "pq:{u1}" "pq:{u1}:m:x" x 1 Payload 2>&1 || true)
