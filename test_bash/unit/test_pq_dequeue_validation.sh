...
suite() {
  ...
  out=$(fcall "$e" pq_dequeue 1 "$q" 1000 30000 2>&1 || true)
  expect_contains "dequeue wrong key count -> EKEYS" "EKEYS" "$out"
  out=$(fcall "$e" pq_dequeue 2 "$q" "$pfx" -1 30000 2>&1 || true)
  expect_contains "negative now -> ENOW" "ENOW" "$out"
  out=$(fcall "$e" pq_dequeue 2 "$q" "$pfx" abc 30000 2>&1 || true)
  expect_contains "non-numeric now -> ENOW" "ENOW" "$out"
  out=$(fcall "$e" pq_dequeue 2 "$q" "$pfx" 1000 0 2>&1 || true)
  expect_contains "zero timeout -> ETMO" "ETMO" "$out"
  out=$(fcall "$e" pq_dequeue 2 "$q" "$pfx" 1000 1.5 2>&1 || true)
  expect_contains "non-integer timeout -> ETMO" "ETMO" "$out"
  out=$(fcall "$e" pq_dequeue 2 "$q" "$pfx" 1000 30000 -1 2>&1 || true)
  expect_contains "negative max_scan -> ESCAN" "ESCAN" "$out"
  out=$(fcall "$e" pq_dequeue 2 "$q" "pq:{u2}:m:" 1000 30000 2>&1 || true)
  ...
  out=$(fcall "$e" pq_ack 2 "$q" "${pfx}y" "$ymember" 999 2>&1 || true)
  expect_contains "ack wrong token -> EFENCED" "EFENCED" "$out"
  out=$(fcall "$e" pq_nack 1 "${pfx}y" 999 2>&1 || true)
  expect_contains "nack wrong token -> EFENCED" "EFENCED" "$out"

  # Settling an absent message -> idempotent NOOP.
  expect "ack absent -> NOOP" "NOOP" "$(fcall "$e" pq_ack 2 "$q" "${pfx}absent" "$(printf '%020d:%s' 9 absent)" 1)"
  expect "nack absent -> NOOP" "NOOP" "$(fcall "$e" pq_nack 1 "${pfx}absent" 1)"
