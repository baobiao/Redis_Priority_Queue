...
for e in $ENGINES; do
  ...
  fcall "$e" pq_enqueue 2 "$q" "${pfx}1" 1 1 Priority 5 Payload work >/dev/null
  ...
  out=$(fcall "$e" pq_nack 1 "${pfx}1" 1 5000)
  ...
  fcall "$e" pq_enqueue 2 "$q" "${pfx}2" 2 1 Priority 5 Payload w2 >/dev/null
  fcall "$e" pq_dequeue 2 "$q" "$pfx" 1000 30000 >/dev/null
  fcall "$e" pq_nack 1 "${pfx}2" 1 >/dev/null           # no delay
  expect "plain nack leaves VisibleAt at default 0" "0" "$(engine_cli "$e" HGET "${pfx}2" VisibleAt)"
  out=$(fcall "$e" pq_dequeue 2 "$q" "$pfx" 1001 30000)
