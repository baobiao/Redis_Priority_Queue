...
suite() {
  ...
  fcall "$e" pq_enqueue 2 "pq:{x1}" "pq:{x1}:m:1" 1 1 Priority 5 >/dev/null
  before=$(engine_cli "$e" ZCARD "pq:{x1}")
  out=$(fcall "$e" pq_enqueue 2 "pq:{x1}" "pq:{x1}:m:1" 1 2 Priority 9 2>&1 || true)
  ...
  out=$(fcall "$e" pq_enqueue 2 "pq:{x2}" "pq:{x2}:m:1" 1 1 Priority 5 2>&1 || true)
  ...
  fcall "$e" pq_enqueue 2 "pq:{x3}" "pq:{x3}:m:1" dup 7 Priority 5 >/dev/null
  before=$(engine_cli "$e" ZCARD "pq:{x3}")
  out=$(fcall "$e" pq_enqueue 2 "pq:{x3}" "pq:{x3}:m:2" dup 7 Priority 9 2>&1 || true)
