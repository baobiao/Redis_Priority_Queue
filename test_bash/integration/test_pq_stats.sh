#!/usr/bin/env bash
# [US2] Integration: pq_stats. Exact depths + front Priority (cheap tier); a
...
suite() {
  ...
  fcall "$e" pq_enqueue 2 "$q" "${pfx}A" A 1 Priority 5  Payload a >/dev/null
  fcall "$e" pq_enqueue 2 "$q" "${pfx}B" B 2 Priority 10 Payload b >/dev/null
  fcall "$e" pq_enqueue 2 "$q" "${pfx}C" C 3 Priority 10 Payload c VisibleAt 999999 >/dev/null
  fcall "$e" pq_enqueue 2 "$q" "${pfx}D" D 4 Priority 20 Payload d >/dev/null
  ...
  fcall "$e" pq_dequeue 2 "$q" "$pfx" 1000 30000 >/dev/null   # leases A (front, available)

  # --- Cheap tier: exact depths + front Priority ---
  out=$(fcall_ro "$e" pq_stats 3 "$q" "$pfx" "$dl" 2000 30000)
  ...
  out=$(fcall_ro "$e" pq_stats 3 "$q" "$pfx" "$dl" 2000 30000 100)
  ...
  out=$(fcall_ro "$e" pq_stats 3 "$q" "$pfx" "$dl" 2000 30000 2)
  ...
  fcall "$e" pq_create 1 "${pfx}X" Priority 5 Payload x DeadLetteredAt 1000 >/dev/null
  engine_cli "$e" ZADD "$dl" 5 "$(printf '%020d:%s' 9 X)" >/dev/null
  out=$(fcall_ro "$e" pq_stats 3 "$q" "$pfx" "$dl" 5000 30000 100)
