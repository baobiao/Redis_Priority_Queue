#!/usr/bin/env bash
# [US1] Contract test for pq_reap: KEYS[1]=DLQ, KEYS[2]=prefix; ARGV=now,
...
suite() {
  ...
  # Empty DLQ -> zero map.
  out=$(fcall "$e" pq_reap 2 "$dl" "$pfx" 1000 1000 10)
  ...
  fcall "$e" pq_create 1 "${pfx}1" Priority 5 Payload x DeadLetteredAt 100 >/dev/null
  m1=$(printf '%020d:%s' 1 1)
  engine_cli "$e" ZADD "$dl" 5 "$m1" >/dev/null
  out=$(fcall "$e" pq_reap 2 "$dl" "$pfx" 100000 1000 10)
  ...
  out=$(fcall "$e" pq_reap 1 "$dl" 1000 1000 10 2>&1 || true)
  expect_contains "wrong key count -> EKEYS" "EKEYS" "$out"

  # Invalid args.
  out=$(fcall "$e" pq_reap 2 "$dl" "$pfx" -1 1000 10 2>&1 || true)
  expect_contains "bad now -> ENOW" "ENOW" "$out"
  out=$(fcall "$e" pq_reap 2 "$dl" "$pfx" 1000 -1 10 2>&1 || true)
  expect_contains "bad retention -> ERET" "ERET" "$out"
  out=$(fcall "$e" pq_reap 2 "$dl" "$pfx" 1000 1000 0 2>&1 || true)
  expect_contains "bad limit -> ELIMIT" "ELIMIT" "$out"

  # Tag mismatch -> ETAG.
  out=$(fcall "$e" pq_reap 2 "dlq:{rc}" "pq:{other}:m:" 1000 1000 10 2>&1 || true)
  ...
  out=$(fcall "$e" pq_reap 2 "str:{rc}" "str:{rc}:m:" 1000 1000 10 2>&1 || true)
