#!/usr/bin/env bash
# [US1] Contract test for pq_create.
...
suite() {
  ...
  out=$(fcall "$e" pq_create 1 "q:{c1}" Payload hello Priority 5)
  expect "create with values returns OK" "OK" "$out"

  out=$(fcall "$e" pq_create 1 "q:{c1}" Payload 2>&1 || true)
  expect_contains "odd ARGV -> EARGS" "EARGS" "$out"

  out=$(fcall "$e" pq_create 1 "q:{c1}" Color red 2>&1 || true)
